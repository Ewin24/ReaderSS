 ...
 * object) and `SanitizerContext.Provider` (the `sanitize` function).
 *
 ...
 * of `worker/**`.
 *
 * This module previously wired only `localStore`/`clock`/`feedSource`.
 * `feedParser` was never constructed or added to `Services`, so
 * `services/subscribeToFeed.ts` and `services/refreshFeeds.ts` -- and
 * therefore the whole feed-ingestion half of the app -- had no real caller
 * anywhere in the production import graph. Confirmed empirically: grepping
 * the built `dist/` bundle before this fix found DOMPurify and `idb` but
 * zero feed-parsing markers (no `pubDate`, no `enclosure`, no
 * `subscribeToFeed`). `feedParser` is now wired below.
 */
